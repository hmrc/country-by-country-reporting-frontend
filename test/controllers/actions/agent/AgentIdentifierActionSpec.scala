...
class AgentIdentifierActionSpec extends SpecBase with TableDrivenPropertyChecks {
  ...
  "Agent Identifier Action" - {
    ...
    "the user doesn't have sufficient enrolments" - {
      ...
      "must redirect the user to the use-agent-services page when AGENT but without AGENT enrolment" in {

        type RetrievalType = Option[String] ~ Enrolments ~ Option[AffinityGroup]
        val authRetrievals = Future.successful(new ~(new ~(Some("id"), Enrolments(Set.empty[Enrolment])), Some(Agent)))
